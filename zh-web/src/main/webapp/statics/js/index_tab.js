...
(function ($) {
    $.zhtab = {
        ...
        refreshTab: function () {
            ...
            var target = $('.ZH_iframe[data-id="' + currentId + '"]');
        ...
        activeTab: function () {
            ...
            if (!$(this).hasClass('active')) {
                $('.mainContent .ZH_iframe').each(function () {
                    if ($(this).data('id') == currentId) {
                        $(this).show().siblings('.ZH_iframe').hide();
                        return false;
                    }
                });
                $(this).addClass('active').siblings('.menuTab').removeClass('active');
                $.zhtab.scrollToTab(this);
            }
        },
        closeOtherTabs: function () {
            $('.page-tabs-content').children("[data-id]").find('.fa-remove').parents('a').not(".active").each(function () {
                $('.ZH_iframe[data-id="' + $(this).data('id') + '"]').remove();
                $(this).remove();
            });
        ...
        closeTab: function () {
            ...
            if ($(this).parents('.menuTab').hasClass('active')) {
                ...
                $.zhtab.scrollToTab($('.menuTab.active'));
            }
            return false;
        },
        addTab: function () {
            ...
            if (flag) {
                ...
                var str1 = '<iframe class="ZH_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
                $('.mainContent').find('iframe.ZH_iframe').hide();
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.zhtab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                scrollVal = $.zhtab.calSumWidth($(tabElement).prevAll());
                if (scrollVal > 0) {
                    $('.page-tabs-content').animate({
        ...
        scrollTabLeft: function () {
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                offsetVal = 0;
                if ($.zhtab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                    while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                        offsetVal += $(tabElement).outerWidth(true);
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.zhtab.calSumWidth($(element).prevAll()),
                marginRightVal = $.zhtab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.zhtab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        init: function () {
            $('.menuItem').on('click', $.zhtab.addTab);
            $('.menuTabs').on('click', '.menuTab i', $.zhtab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.zhtab.activeTab);
            $('.tabLeft').on('click', $.zhtab.scrollTabLeft);
            $('.tabRight').on('click', $.zhtab.scrollTabRight);
            $('.tabReload').on('click', $.zhtab.refreshTab);
            $('.tabCloseAll').on('click', function () {
                ...
                $('.page-tabs-content').children("[data-id]:first").each(function () {
                    $('.ZH_iframe[data-id="' + $(this).data('id') + '"]').show();
                    $(this).addClass("active");
                });
                $('.page-tabs-content').css("margin-left", "0");
            });
            $('.tabCloseOther').on('click', $.zhtab.closeOtherTabs);
            $("#sidebar-menu li a").click(function () {
                ...
                if (e.is(".treeview-menu") && e.is(":visible")) {
                    ...
                    var h = d.parent("li");
                    var _height3 = $.zhtab.calSumHeight($("#sidebar-menu>li"));
    ...
    $.zhindex = {
        load: function () {
            ...
            $("#content-wrapper").find('.mainContent').height($(window).height() - 122);
        ...
        loadMenu: function (data) {
            ...
            $.each(data, function (i) {
                ...
                if(row.type == 0) {
                    ...
                    _html += $.zhindex.getChild(row.list);
                    _html += "</ul>";
                }
                if(row.type == 1) {
                    if(row.icon == null){
...
var vm = new Vue({
	...
	methods : {
		...
		fullScreen: function() {
            if (!$('#fullscreen').attr('fullscreen')) {
                ...
                $.zhtab.requestFullScreen();
            } else {
                $('#fullscreen').removeAttr('fullscreen')
                $.zhtab.exitFullscreen();
            }
        },
	...
	updated : function() {
		$.zhindex.load();
        $.zhindex.loadMenu(this.menuList);
        $.zhtab.init();
        setScroll();
	}
});

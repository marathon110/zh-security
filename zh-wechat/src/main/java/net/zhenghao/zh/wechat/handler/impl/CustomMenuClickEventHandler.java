...
import net.zhenghao.zh.wechat.message.request.CustomMenuClickEventRequestMessage;
...
public class CustomMenuClickEventHandler implements MessageHandler {
    ...
    public BaseResponseMessage dealMessage(BaseRequestMessage requestMessage) {
        //自定义菜单点击事件业务逻辑
        CustomMenuClickEventRequestMessage customMenuClickEventRequestMessage = (CustomMenuClickEventRequestMessage) requestMessage;
        if ("my".equals(customMenuClickEventRequestMessage.getEventKey())) {
            WechatUserEntity user = UserUtils.getUserInfo(customMenuClickEventRequestMessage.getFromUserName(), Lang.CHINESE.getCode());
            if (user == null) {
                return MessageUtils.buildTextResponseMessage(customMenuClickEventRequestMessage, "抱歉,没有获取到您的信息,请您稍后再重试.");
            }
            ...
            return MessageUtils.buildTextResponseMessage(customMenuClickEventRequestMessage, userInfoString);
        }
        return null;
    }
}
